...
import { uploadCollectionsProduct } from "../../../../lib/store/redux/adminSlice";
import { motion, AnimatePresence } from "framer-motion";
import { Plus, Package, Trash2, ListPlus } from "lucide-react";
...
export default function CollectionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await dispatch(uploadCollectionsProduct(data) as any).unwrap();
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          ...
          <Package size={40} className="text-zinc-200 hidden md:block" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="space-y-8">
              <div>
                ...
                <textarea
                  ...
                  placeholder="GENERAL PRODUCT OVERVIEW..."
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {["frontImage", "backImage", "leftImage", "rightImage"].map(
                (view) => (
                  <div
                    key={view}
                    className="relative aspect-[3/4] bg-zinc-100 border-2 border-zinc-200 group overflow-hidden"
                  >
                    <input
                      type="file"
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                      {files[view] ? (
                        <div className="w-full h-full flex flex-col items-center justify-center bg-white">
                          <Package size={32} className="mb-2 text-black" />
                          <p className="text-[8px] font-bold uppercase truncate w-full px-2">
                            {files[view]!.name}
                          </p>
                          <button
                            type="button"
                            onClick={() => setFiles({ ...files, [view]: null })}
                            className="mt-2 text-[10px] underline font-bold text-red-500"
                          >
                            REMOVE
                          </button>
                        </div>
                        ...
                        <>
                          <Plus
                            size={20}
                            className="mb-2 group-hover:scale-125 transition-transform text-zinc-400 group-hover:text-black"
                          />
          ...
          {/* Submit Section */}
          <div className="flex flex-col items-end gap-4 pt-8 border-t-2 border-zinc-100">
            <AnimatePresence mode="wait">
              {status && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
            ...
            <button
              disabled={isLoading}
              className={`w-full md:w-auto bg-black text-white px-16 py-6 font-black uppercase tracking-[0.4em] transition-all active:scale-95 shadow-2xl flex items-center justify-center gap-4 ${isLoading ? "opacity-50 cursor-not-allowed" : "hover:bg-zinc-800"}`}
            >
              {isLoading ? (
                <>
